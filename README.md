...
- **Scheduling patch.** Upstream's kind manifest selects only `kubernetes.io/os=linux`. With worker
  nodes present the controller can be scheduled on a worker, where no host port leads, and
  `curl http://localhost:$INGRESS_HTTP_HOST_PORT` gets "Empty reply from server". The kustomization
  pins the controller to the `ingress-ready=true` node.
